...
const VideoConsultation = () => {
  ...
  useEffect(() => {
    try {
      ...
      if (decryptedParams) {
        ...
        const roomId = parsedParams.app_no;
  ...
  const initializeZego = async () => {
    try {
      ...
      try {
        ...
        onUserUpdate: (updateType, userList) => {          
          // Update participant count based on the actual user list
        ...
        <div style={{
          ...
          <div style={{
            ...
            <ul style={{
              ...
              <li><strong>Room:</strong> {appointmentData?.roomID}</li>
            </ul>
          </div>
  ...
  // Update the status display logic to use real-time participant count
  const updateStatusDisplay = () => {
    // Use the correct selectors that match your actual DOM elements
    const statusText = document.getElementById('status-text');
    const participantsList = document.getElementById('participants-list');
    ...
    // Use the current participant count from state
    ...
    if (currentCount === 0) {
      ...
      statusText.textContent = '🟢 1 participant in the room';
      participantsList.style.display = 'flex';
    } else {
      statusText.textContent = `🟢 ${currentCount} participants in the room`;
      participantsList.style.display = 'flex';
    }
  };

  // Call this function whenever participant count changes
